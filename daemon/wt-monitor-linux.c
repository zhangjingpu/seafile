               ...
               gboolean last_event)
{
    ...
    if (!rename_info->processing) {
        if (event->mask & IN_MOVED_FROM) {
            ...
            add_event_to_queue (status, WT_EVENT_ADD_RECURSIVE, filename, NULL);
            /* Add watch but don't produce events. */
        ...
        if (event->mask & IN_MOVED_FROM) {
            ...
            if (event->cookie == rename_info->last_cookie) {
                ...
                add_event_to_queue (status, WT_EVENT_ADD_RECURSIVE,
                                    filename, NULL);
                add_watch_recursive (info, in_fd, worktree, filename, FALSE);
            }
            unset_rename_processing_state (rename_info);
...
static int handle_rm_repo (SeafWTMonitorPriv *priv,
                           const char *repo_id,
                           gpointer handle)
{
